...
function Broadcast(props) {
    const {handleFormSubmit, handleInputChange, textareaValue, 
        countryType, gridType, showTags, showBrand, showKlarna, 
        products, badges, setBadge, certonaTag} = props;
    return (
    <div className="rows">
        ...
        <form onSubmit={handleFormSubmit} id="linkgens">
          <div>
            <label htmlFor="links">
              ...
              <br />
              <br />
              <textarea rows="24" cols="100" id="links" name="textareaValue" value={textareaValue} onChange={handleInputChange} />
            </label>
          </div>
                  ...
                  {products.map((product, index) => {
                    ...
                    return (
                      <React.Fragment key={product.skuId}>
                        <li key={product.skuId}><b>Pick {index + 1}</b>: {product.brandName}</li>
                        <ul>
                          {badges.map((badge, badgeIdx) => {
                            ...
                            return (
                              <li key={product.skuId + badge.name}>
                                <label htmlFor={id}>
                                  <input type="radio" id={id} name={name} checked={product.badge === badge} onChange={() => setBadge(index, badge)} />
                                  {' '}
                                  {badge.name}
                                </label>
                              </li>
